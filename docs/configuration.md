...
| `OPENAI_CODEX_MODEL` | no | _(falls back to `OPENAI_MODEL_CODING`)_ | **Deprecated legacy variable.** Still active in the internal model resolution chain (checked after `OPENAI_MODEL_CODING`). Omitted from `.env.example` intentionally — new configurations should use `OPENAI_MODEL_CODING` instead. Set only if you have existing scripts or automation that already use this variable. | `gpt-4o` |
