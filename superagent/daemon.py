...
def run_daemon(*, poll_interval_seconds: int = 30, heartbeat_interval_seconds: int = 300, once: bool = False) -> int:
    ...
    log_task_update("Daemon", f"Superagent daemon started. poll={poll_interval_seconds}s heartbeat={heartbeat_interval_seconds}s")
        ...
        if heartbeat_agent and (
            ...
            try:
                heartbeat_agent.handler(
                    {
                        **_daemon_state(),
                        "heartbeat_service_name": "superagent-daemon",
    ...
    log_task_update("Daemon", "Superagent daemon finished.")
    return 0
