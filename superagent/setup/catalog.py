...
INTEGRATION_DEFINITIONS: tuple[IntegrationDefinition, ...] = (
    ...
    IntegrationDefinition(
        ...
        fields=(
            _field("SUPERAGENT_PRIVILEGED_MODE", "Privileged Mode", "If true, privileged policy controls are enabled for runs."),
            _field("SUPERAGENT_REQUIRE_APPROVALS", "Require Approvals", "If true, privileged actions require explicit approval."),
            _field("SUPERAGENT_READ_ONLY_MODE", "Read-Only Mode", "If true, mutating command and file actions are blocked."),
            _field("SUPERAGENT_ALLOW_ROOT", "Allow Root Escalation", "If true, sudo/root escalation is allowed for privileged runs."),
            _field("SUPERAGENT_ALLOW_DESTRUCTIVE", "Allow Destructive Commands", "If true, destructive operations may run."),
            _field("SUPERAGENT_ENABLE_BACKUPS", "Enable Snapshots", "If true, snapshots are created before mutating OS commands."),
            _field("SUPERAGENT_ALLOWED_PATHS", "Allowed Paths", "Comma or pathsep-separated allowlist roots for privileged scope."),
            _field("SUPERAGENT_ALLOWED_DOMAINS", "Allowed Domains", "Comma-separated allowed network domains for privileged tasks."),
            _field("SUPERAGENT_KILL_SWITCH_FILE", "Kill Switch File", "If this file exists, runtime halts before more agent execution."),
        ),
    ...
    runtime_components = [
        {
            ...
            "fields": [
                asdict(_field("SUPERAGENT_HOME", "Superagent Home", "Home directory for plugin and config files.")),
                asdict(_field("SUPERAGENT_PLUGIN_PATHS", "Plugin Paths", "OS path-separated plugin paths.")),
                asdict(_field("OUTPUT_DIR", "Output Directory", "Output folder for runs and setup artifacts.")),
                asdict(_field("SUPERAGENT_WORKING_DIR", "Working Folder", "Required base folder for task runs, artifacts, and outputs.", required=True)),
